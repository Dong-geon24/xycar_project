...
def draw_line_and_intersection(img, a, b, c, d):
    ...
    # y=c인 중간 선을 그립니다.
    cv2.line(img, (0, mid_y), (img.shape[1] - 1, mid_y), (0, 255, 0), 2)

    # 왼쪽 차선의 교점을 계산하고 원을 그립니다.
    if a is not None and b is not None:
        intersection_x = int((mid_y - b) / a)
        intersection_y = mid_y
        cv2.circle(img, (intersection_x, intersection_y), 5, (255, 0, 0), -1)
    else:
        cv2.circle(img, (112, mid_y), 5, (255, 0, 0), -1)

    # 오른쪽 차선의 교점을 계산하고 원을 그립니다.
    if c is not None and d is not None:
        intersection_w = int((mid_y - d) / c)
        intersection_z = mid_y
        max_value = 1000  # 혹은 다른 유효한 범위의 값으로 설정하세요
        intersection_w = min(max(intersection_w, -max_value), max_value)
        intersection_z = min(max(intersection_z, -max_value), max_value)
        cv2.circle(img, (intersection_w, intersection_z), 5, (255, 0, 0), -1)
    else:
        cv2.circle(img, (448, mid_y), 5, (255, 0, 0), -1)
...
def roi(dst):
    ...
    y = int(dst.shape[0])
    # 범위지정
    section = np.array([[40, 200],[40, 1000], [600, 1000],[600, 200] ])
    # 마스크 생성
...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        if lines is not None:
            ...
            for line in lines:
                ...
                if slope < 0:  # 왼쪽 차선
                    ...
                    lf_x = left_x_coords if left_x_coords else []
                    lf_y = left_y_coords if left_y_coords else []

                else:  # 오른쪽 차선
                    right_x_coords.extend([x1, x2])
                    right_y_coords.extend([y1, y2])

                    rt_x = right_x_coords if right_x_coords else []
                    rt_y = right_y_coords if right_y_coords else []
            ...
            if len(right_x_coords) > 1 and len(right_y_coords) > 1:
                ...
                if prev_coefficients is not None:
                    ...
                    c = d = 0  # 예외 처리: 이전 계수도 없는 경우

            # 화면에 직선과 교점을 그립니다.
            img_with_lines = draw_line_and_intersection(transformed_img, a, b, c, d)
            ...
            if not np.isnan(left_lane_avg[0]) and not np.isnan(right_lane_avg[0]):
                ...
                mid_y = img.shape[0] * 3 // 5
                cv2.circle(img_with_lines, (avg_x, mid_y), 5, (0, 255, 0), -1)
        ...
        # 결과 이미지 출력
        cv2.imshow("Camera View", img_with_lines)
